...
AI 编排采用有边界的 LangGraph 工作流，分别执行诊断、创意、投放建议和复盘。商品运营轮次和人工审批由业务服务管理，跨数日的投放周期不通过一个长期运行的模型会话维持。

本文的技术选型、数据结构、状态扩展及参数是架构建议，并非原文已指定或已经实现的事实。需求未明确的业务口径保留决策项，并提供可实现的建议方案。本次交付为设计文档，不包含应用实现和性能实测。
...
| AI 编排 | LangGraph，有类型的 State | 表达生成、校验、有限修复和提交过程 |
...
LangGraph 承担四种受控任务：商品诊断、创意生成、投放建议、经营复盘。它不维护库存，不执行广告投放，不代替人工审批。没有必要为每一个业务表创建 Agent，也不需要让多个 Agent 自由对话来推进明确的业务流程。
...
每次任务 attempt 使用独立 thread_id，建议为 task_id:attempt_id。第一版可以从输入快照重新执行短图；如启用 PostgreSQL Checkpointer，检查点只用于节点恢复，不作为审批和业务事实来源。persist 使用 task_id 与产物类型唯一约束，避免检查点重放造成重复落库。
