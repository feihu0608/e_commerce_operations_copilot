...
| AI 编排与契约 | DiagnosisGraph、CreativeGraph、StrategyGraph、ReviewGraph 使用 LangGraph Typed State；Pydantic 校验，真实模型输出允许一次有限修复 |
| 视频任务 | submit 和 poll 拆成短 Celery 任务，保存 provider_job_id；Dispatcher 恢复过期轮询 |
| 内容版本 | 诊断、创意、复盘创建内容修订；AI 投放建议创建关联 task_id 的草稿，结果精确指向所属产物 |
...
AI 编排采用四类有边界的 LangGraph StateGraph，分别执行诊断、创意、投放建议和复盘；媒体生成继续使用适合供应商异步任务的 Celery submit/poll 状态机。商品运营轮次和人工审批由确定性业务服务管理，跨数日的投放周期不通过一个长期运行的模型会话维持。
...
flowchart TB
    ...
    AI --> GRAPH[LangGraph Typed State 与模型适配层]
    GRAPH --> LLM[大语言模型服务]
...
| AI 编排 | LangGraph StateGraph、有类型的 GenerationState、PostgreSQL Checkpointer | 表达加载、生成、校验、有限修复和持久化过程；检查点不代替业务事实 |
...
ecommerce-operations-assistant/
  ...
  backend/app/
    main.py                仅暴露 ASGI app 的薄兼容入口
    tasks.py               仅暴露 Celery registry 的薄兼容入口
    dispatcher.py          仅启动 Dispatcher 的薄兼容入口
    api/                   FastAPI 应用、路由和 HTTP Schema
    core/                  配置、安全、请求上下文与可观测性
    domain/                SQLAlchemy 领域模型与 AI 输出契约
    infrastructure/        数据库会话及后续队列/存储适配
    integrations/          SiliconFlow 等外部供应商网关
    services/              任务、Outbox、审计等应用服务
    workflows/
      state.py             Typed GenerationState 与运行上下文
      catalog.py           四类工作流提示契约和显式 Mock 样例
      nodes.py             load/generate/validate/repair/persist 节点
      graph.py             StateGraph 拓扑、条件边和调用入口
      checkpoint.py        PostgreSQL/InMemory Checkpointer 选择
    workers/
      runtime.py           Celery 配置和模型调用记录
      content_tasks.py     四类 LangGraph 文本任务
      media_tasks.py       图片与视频 submit/poll 任务
      tasks.py             Worker 内部任务注册薄入口
      dispatcher.py        Outbox 分发与租约恢复
  migrations/
...
LangGraph 承担四种受控任务：商品诊断、创意生成、投放建议和经营复盘。统一节点链为 `load_context → generate → validate → persist`；校验失败且仍有额度时通过条件边进入一次 `repair → validate`，耗尽后进入失败终态。它不维护库存，不执行广告投放，不代替人工审批。Pydantic 完成结构校验，业务服务完成权限、状态转换与最终写入。没有必要为每一个业务表创建 Agent，也不允许多个 Agent 自由对话推进明确业务流程。
...
API 验证用户和商品后保存不可变输入快照；Worker 读取任务和 attempt 创建 State。load_context 填充证据引用和提示词，generate 保存原始输出，validate 生成 Pydantic candidate 或错误，repair 更新修复计数，persist 写入内容修订或投放草稿并返回 output_revision_id。State 不携带密钥、原始图片字节或完整视频。

每次执行创建独立 task_attempt，并由 task_id、attempt_no 唯一约束；LangGraph thread_id 固定为 `task:{task_id}:attempt:{attempt_id}`。生产环境使用 PostgreSQL Checkpointer 保存节点恢复状态，测试显式注入 InMemorySaver。检查点只用于节点恢复，不作为审批和业务事实来源。Worker 只领取 pending 任务，所有终态更新同时检查 task 与 attempt 仍为 running；内容或投放建议通过 task_id 唯一关联，避免重复消息或迟到执行错误落库。
...
| ADR-03 AI 编排 | 四类 LangGraph StateGraph、Typed State、统一模型入口与 PostgreSQL Checkpointer | 节点恢复和路径可追踪，增加框架与检查点表维护成本 |
...
设计追踪依据为需求文档第 1—13 节。架构基线受 `architecture-baseline.json` 和 CI 检查保护；当前基线变更为 ACP-0002。守卫同时验证文档哈希、关键实现证据、必需目录、薄入口最大行数和禁止回流的根目录模块。框架、状态权威、部署拓扑、安全边界、核心工作流或模块边界若需改变，必须先提交 ACP 并取得用户明确批准，禁止通过静默修改文档追认实现偏差。
